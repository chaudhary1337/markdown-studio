...
- Bug fix: `**\`bold code\`**` no longer loses its bold wrapper after a round-trip through the rich editor. Cause: Tiptap's default `Code` mark sets `excludes: '_'`, which strips every other mark when code is applied — so `<strong><code>…</code></strong>` was parsed as plain `<code>…</code>` on the way in. Disabled StarterKit's bundled Code mark and re-added it with `excludes: ''` so inline code can coexist with bold/italic. Closes #3.
