...
- Feature: New `Markdown Studio: Rerun First-Run Setup` command clears the `betterMarkdown.consentShown` globalState flag so the on-open welcome modal fires again on the next file open. Useful for testing the first-run flow and for users who want to revisit the welcome prompt without poking the SQLite store directly.
