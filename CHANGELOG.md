...
- Bug fix: Checkbox diffs in the rendered diff view now show the whole task line on both sides — `[ ] foo` → `[x] foo` — instead of just the checkbox input wrapped in `<del>`/`<ins>` with the trailing label sitting unchanged. The pre-process pass now collapses the entire `<li.task-list-item>` body into a single `<video>` sentinel, with the original HTML stashed (encoded) on a `data-btrmk-html` attribute so post-processing can put it back verbatim. The visible inner text of the sentinel is "☐ <line>" / "☑ <line>", which makes htmldiff treat the line as one atomic unit — any change (checkbox state, label text, or both) gets the full red-strike / green-underline pair.
